package com.example.homework2.domain.util
...
sealed class HandleErrorStates(val errorCode: ErrorCode) {
    ...
    data object ServerError : HandleErrorStates(ErrorCode.SERVER_ERROR)
    data class HttpError(val httpCode: Int, val errorBody: String = "") : HandleErrorStates(
        ErrorCode.HTTP_ERROR
    )
    data object NetworkError : HandleErrorStates(ErrorCode.NETWORK_ERROR)
    data object TimeoutError : HandleErrorStates(ErrorCode.TIMEOUT_ERROR)
    data object UnknownError : HandleErrorStates(ErrorCode.UNKNOWN_ERROR)
